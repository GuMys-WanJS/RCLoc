...
from util.cl_tool import remove_noise_threshold
...
def run(trainFile, trainLabelFile, testFile, testLabelFile, loss, filepath, threshold):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        predictions = sess.run(tf.nn.softmax(pred), feed_dict={spec: datasets.train.instances[:, :34],
    ...
    train_instances, train_labels = remove_noise_threshold(labels, predictions, datasets.train.instances,
